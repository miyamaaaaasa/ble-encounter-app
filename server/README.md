...
| タブ | 内容 |
|---|---|
| 状況 | アクティブ利用者数（5分/1時間/24時間）・ホストのCPU/メモリ/ディスク・バックアップ世代 |
| 利用者 | 表示名・ドット絵・自己紹介・最終アクセス（30秒ごとに自動更新） |
| 配信 | お知らせの送信 |
...
| メソッド | パス | 認証 | 用途 |
|---|---|---|---|
| POST | `/admin/api/broadcast` | Basic + `X-Admin-Token` | お知らせを1件配信 |
| GET | `/admin/api/stats` | 同上 | 稼働状況・負荷・バックアップ一覧 |
| GET | `/admin/api/users` | 同上 | 利用者のプロフィール一覧 |
| POST | `/admin/api/backup` | 同上 | その場でバックアップを作成 |
| POST | `/admin/broadcast` | 同上 | 旧パス（既存スクリプト互換で残置） |
...
外部の監視エージェントを常駐させず、APIコンテナ自身が `/proc/stat`・`/proc/meminfo`・
`statfs` を読んで算出する（Dockerでは `/proc` がホストのものなので**ホスト全体**の
負荷が取れる）。CPUは瞬間値が取れないため5秒間隔でサンプリングした最新値を返す。

アクティブ人数は `users.last_seen_at` で判定する。認証付きAPIへのアクセス時に
更新するが、**毎回書くと負荷になるため60秒に1回まで間引いている**。
記録するのは時刻のみで、IP・行動履歴・すれ違い相手は一切残さない。

### バックアップボタンの範囲

画面のボタンが作るのは**サーバー内のローカル複製のみ**。Google Driveへの退避は
ホスト側のcron(毎日4:10)が担当で、コンテナからは起動できない
（起動するにはコンテナへdockerソケットを渡す必要があり、共有サーバーでは危険）。
即座にDriveへ上げたい場合はSSHで `sh ~/hajimemashite/server/backup/backup.sh` を実行する。

### 表示する利用者データの範囲（プライバシー）

アプリの絶対維持事項（匿名性）を崩さないため、管理画面に出すのは
**利用者がすれ違った相手に見せる前提で設定した情報**に限る。

| 表示する | 表示しない |
|---|---|
| 表示名・アバター色・ドット絵 | APIキー（ハッシュのみ保持） |
| 自己紹介（選択肢インデックス） | すれ違いの相手・履歴・時刻 |
| 最終アクセス時刻・ユーザーID先頭8文字 | IPアドレス・位置情報（そもそも収集していない） |

自己紹介は自由入力ではなく**選択肢のインデックス**で保持する。
個人を特定しうる自由文がサーバーに残らない。
